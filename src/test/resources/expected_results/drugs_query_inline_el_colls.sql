select -- rows of XDAGENTEST.DRUG
  ...
  xmlserialize(content xmlelement(name "drug", xmlattributes('http://example/namespace' as "xmlns")
   ...
   ,(select xmlagg(dfc_rows.row_xml) "rowcoll_xml"
     from
     ...
     where
       dfc_rows.DRUG_ID = d.ID
    ) -- child subquery
   ,(select xmlagg(a_rows.row_xml) "rowcoll_xml"
     from
   ...
   ,(select xmlagg(dr_rows.row_xml) "rowcoll_xml"
     from
     ...
     where
       dr_rows.DRUG_ID = d.ID
    ) -- child subquery
   ,(select xmlagg(b_rows.row_xml) "rowcoll_xml"
     from
